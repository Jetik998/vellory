...
async def get_current_user_cookie(request: Request, session: SessionDep):
    token = request.cookies.get("access_token")

    if token is None:
        raise UnauthorizedException("Access token missing.")

    token_data: TokenData = verify_token_cookie(token)
...
def verify_token_cookie(token):
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        email = payload.get("sub")
        token_type: str | None = payload.get("token_type")

        if email and token_type == TokenType.ACCESS:
...
CurrentUserFromCookie = Annotated[User, Depends(get_current_user_cookie)]
